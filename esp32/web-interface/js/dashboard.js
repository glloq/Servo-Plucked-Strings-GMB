    ...
    return {
      ...
      strings: p.strings.map(function (s, i) {
        return { index: i, state: '—', note: null, fret: null,
          finger: '—', plectrum: '—', lastFault: 'none', openNote: s.openNote };
      })
    };
  }

  function buildHead() {
    // Servo-per-fret: no carriage position / HOME / LIMIT columns.
    var cols = ['#', 'Open', 'State', 'Note', 'Fret', 'Finger', 'Plectrum', 'Last fault'];
  ...
  function update(st) {
    ...
    (st.strings || []).forEach(function (s) {
      tb.appendChild(h('tr', [
        ...
        h('td', s.note === null || s.note === undefined ? '—' : GMB.noteName(s.note)),
        h('td', s.fret === null || s.fret === undefined ? '—' : String(s.fret)),
        h('td', s.finger || '—'),
        h('td', s.plectrum || '—'),
