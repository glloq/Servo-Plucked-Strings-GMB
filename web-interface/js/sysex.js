...
(function (global) {
  ...
  // Non-persisted advanced toggles for the deprecated fixed-block path. Polyphony
  // is now a real, persisted profile field (instrument.polyphonyMax), not a toggle.
  var adv = { block5: true, block6: true, block7: true, block7Version: 1, detectionEnabled: true };
  ...
  function render(host) {
    ...
    host.appendChild(h('div.card', [
      ...
      h('div.grid.caps-grid', [
        ...
        capItem('Polyphony', caps.polyphony + (caps.polyphonyAuto ? ' (auto)' : ' (custom)')),
    ...
    if (GMB.isAdvanced()) {
      host.appendChild(h('div.card', [
        ...
        h('div.form-grid', [
          ...
          GMB.field('Polyphony (0 = automatic)', GMB.input(p.instrument, 'polyphonyMax', {
            type: 'number', min: 0, max: 6, onChange: function () { GMB.render(); } }), 'announced max simultaneous notes')
        ]),
    ...
    host.appendChild(h('div.card', [
      h('h2', 'SysEx tester'),
      h('div.toolbar.wrap', [
        GMB.button('GMB descriptor (v2)', showDescriptor, 'primary'),
        GMB.button('Request identity (v2 handshake)', function () { runSysEx('identity'); }),
        GMB.button('Notify change', function () { runSysEx('notify'); }),
        GMB.button('Request capabilities (v1)', function () { runSysEx('capabilities'); }),
        GMB.button('Request string config (v1)', function () { runSysEx('stringConfig'); }),
