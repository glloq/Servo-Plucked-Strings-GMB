  ...
  function render(host) {
    ...
    if (!strings.length) {
      host.appendChild(h('div.card', [h('div.pill.warn', 'No strings configured yet.'),
        h('p.muted', 'Define the instrument in the Setup Wizard first.'),
    ...
    if (!anyStriker)
      host.appendChild(h('div.note-box', [
        ...
        GMB.button('Configure plucking', function () { GMB.navigate('wizard'); }, 'ghost')]));
    else if (!anyFinger)
      host.appendChild(h('div.note-box', [
        'No finger servos are equipped — only the open strings can be played. ',
        GMB.button('Equip frets', function () { GMB.navigate('wizard'); }, 'ghost')]));
    ...
    host.appendChild(h('div.card', [
      ...
      h('div.row', [
        ...
        GMB.button('STOP (panic)', function () { GMB.doPanic && GMB.doPanic(); }, 'danger')
      ])
    ]));
  }
