...
struct InstrumentView {
    ...
    // Per string, bit F set = fret F carries a finger servo (fret 0 = open, always
    // playable, not represented). Empty => treat every fret 1..maxFret as available
    // (back-compat / tests that don't wire servos).
    std::vector<uint32_t> availableFretMask;
    int8_t capo = 0;
    int8_t transpose = 0;
    ...
    // True if this exact fret can be fretted on this string: the open string
    // (fret 0) always, otherwise the fret must be within range AND carry a servo.
    // When no mask is provided, any fret within range counts as available.
    bool fretAvailable(uint8_t stringIndex, uint8_t fret) const {
        if (fret == 0) return true;
        if (fret > maxFret(stringIndex)) return false;
        if (stringIndex >= availableFretMask.size()) return true;  // no mask -> range only
        if (fret > kMaxFret) return false;
        return (availableFretMask[stringIndex] >> fret) & 1u;
    }
};
